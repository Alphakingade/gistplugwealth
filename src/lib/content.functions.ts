 ...
 *
 * These are plain async functions that talk to the database directly from the
 * browser using the publishable key. Row level security decides what an
 * anonymous visitor may read or write. Call signatures keep the `{ data }`
...
export async function getArticle({ data: input }: { data: unknown }) {
  ...
  const { data: article, error } = await supabase
    .from("articles")
    .select(
      `${CARD_FIELDS},content,seo_title,seo_description,category_id,updated_at,article_tags(tags(id,name,slug))`,
    )
    .eq("slug", slug)
    .eq("status", "published")
    .maybeSingle();
